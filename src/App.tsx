...
import { Calendar as CalendarIcon, House, List, Menu, Settings, SquareCheck, Users, UtensilsCrossed, X } from 'lucide-react';
...
export default function App() {
  ...
  return (
    ...
    >
      <div className={`flex h-full transition-opacity ${isDimmed() ? 'opacity-30' : 'opacity-100'}`}>
        {/* Sidebar - Desktop */}
        <nav className="hidden md:flex flex-col w-72 h-full dark:bg-slate-900 bg-white border-r border-gray-50 dark:border-slate-800 shadow-sm">
          <div className="p-6 flex items-center justify-center">
            <h1 className="text-2xl font-bold bg-gradient-to-r from-indigo-500 to-purple-600 bg-clip-text text-transparent" style={{ fontFamily: 'Poppins, sans-serif' }}>
              FamilyHub
            </h1>
          </div>
        ...
        <div className="md:hidden fixed top-0 inset-x-0 h-16 bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between px-6 z-30 shadow-sm">
          <h1 className="text-xl font-bold bg-gradient-to-r from-indigo-500 to-purple-600 bg-clip-text text-transparent" style={{ fontFamily: 'Poppins, sans-serif' }}>
            FamilyHub
          </h1>
...
function NavLink({ to, icon, label }: NavLinkProps) {
  ...
  return (
    ...
    >
      <span className={isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'}>{icon}</span>
      <span>{label}</span>
    </Link>
  );
}
