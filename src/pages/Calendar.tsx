...
import { Calendar, ChevronLeft, ChevronRight, Clock, Filter, ListFilter, Plus, Repeat, Tag, Users, X } from 'lucide-react';
      ...
      <div className="relative">
        {hours.map(hour => {
          ...
          return (
            <div 
              key={hour} 
              className="border-t border-gray-200 dark:border-gray-700 h-16 relative"
            >
                    ...
                    <div className="flex items-start">
                      ...
                      <div className="flex-1 ml-4">
                        <div className="flex items-center">
                          ...
                          {event.isRecurring && (
                            <Repeat size={14} className="ml-2 text-indigo-500 dark:text-indigo-400" />
                          )}
                        </div>
          ...
          <div className="h-full overflow-y-auto">
            <div className="grid grid-cols-7 min-h-[1000px] relative">
              ...
              <div className="absolute inset-0 grid grid-cols-7">
                {days.map((day, i) => (
                  <div key={i} className="relative border-r border-gray-100 dark:border-slate-800 last:border-r-0">
                    {renderTimeGrid(day)}
                      ...
                      {dayEvents
                        ...
                        .map((event, eventIndex) => {
                          ...
                          return (
                            ...
                            >
                              <div className="font-medium truncate">{event.title}</div>
                              <div className="text-[10px] text-gray-600 dark:text-gray-400">All day</div>
                            </div>
                          );
                        })}
                    </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-50 dark:border-slate-800 overflow-hidden h-[calc(100vh-220px)]">
          <div className="grid grid-cols-7 border-b border-gray-100 dark:border-slate-800">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, i) => (
              <div key={i} className="p-2 text-center text-xs font-medium text-gray-500 dark:text-gray-400">
                {day}
              </div>
            ))}
          </div>
          ...
          <div className="h-full overflow-y-auto">
            {weeks.map((week, weekIndex) => (
              <div key={weekIndex} className="grid grid-cols-7 border-b border-gray-100 dark:border-slate-800 last:border-b-0">
              ...
              <div className="flex flex-wrap gap-2">
                {EVENT_TYPES.map(type => (
                  <button
                    ...
                    style={{ 
                      backgroundColor: eventTypeFilter.includes(type.value) ? `${type.color}20` : '',
